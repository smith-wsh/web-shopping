...
public class EditGoods implements GoodsImplement {
	private Connection conn = null;

	private PreparedStatement pstmt = null;

	public EditGoods(Connection conn) {
		this.conn = conn;
	}
	...
	public boolean addGoods(Goods goods) throws Exception {
		pstmt = null;
		String sql = "insert into goods(gname,gphoto,types,producer,price,carriage,pdate,paddress,described,number,gid)value(?,?,?,?,?,?,?,?,?,?,?);";
		int result = 0;
	...
	public List<Goods> getAllGoods() throws Exception {
		...
		while (rs.next()) {
			...
			goods = new Goods(gid, gname, number, photo, type, producer, price,
